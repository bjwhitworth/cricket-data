...
deliveries_with_running_totals as (
  ...
  from {{ ref('int_cricket__deliveries_flattened') }} d
)
...
select
  ...
  case
    ...
    when (ic.target_runs - dwrt.runs_so_far) * 6.0 / 
...
from deliveries_with_running_totals dwrt
left join innings_context ic
  on dwrt.match_id = ic.match_id
  and dwrt.innings_number = ic.innings_number
left join match_config mc
